...
public class AdminController {
    ...
    @PreAuthorize("hasRole('ADMIN')")
    public @ResponseBody
    ResponseEntity<?> createNews(@Valid @RequestBody NewsArticleDTO newsArticleDTO, BindingResult bindingResult) {
    ...
    @PreAuthorize("hasRole('ADMIN')")
    public @ResponseBody
    ResponseEntity<?> updateNews(@Valid @RequestBody NewsArticleDTO newsArticleDTO, BindingResult bindingResult) {
        if (bindingResult.hasErrors()) {
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST);
        }
        ...
        HttpResponse<String> result;
        try {
            // Make UPDATE request to delete a new NewsArticle/Post on the Flask news microservice
            HttpRequest updateNews = HttpRequest.newBuilder()
            ...
            result = HttpClient.newBuilder()
                    .build()
                    .send(updateNews, HttpResponse.BodyHandlers.ofString());
        } catch (IOException | InterruptedException e) {
            LOGGER.error(e.getMessage());
            throw new ResponseStatusException(HttpStatus.INTERNAL_SERVER_ERROR);
        }
